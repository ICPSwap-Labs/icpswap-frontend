...
const escapeSvgs = (html: string): string => html.replace(/<svg[^>]*>[\s\S]*?<\/svg>/gi, escapeHtml);
...
const transformImg = (img: string): string => {
  ...
  const shouldEscape = isUndefinedOrNull(src) || src.startsWith("data:image");
...
export const htmlRenderer = (html: string): string => (/<img\s+[^>]*>/gi.test(html) ? transformImg(html) : html);
...
export function Markdown({ content }: MarkdownProps) {
  ...
  return html ? <div className="markdown" dangerouslySetInnerHTML={{ __html: sanitize(html) }} /> : null;
}
