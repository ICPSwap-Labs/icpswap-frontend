  ...
  return (
    <Box sx={{ width: "100%", padding: "0 25px" }}>
      <Box sx={{ width: "100%", overflow: "auto", border: "1px solid #29314F", borderRadius: "16px" }}>
        <PieChartTitle
          content0={t("info.swap.tokens.holders.percent", {
            percent: top100HoldPercent ? `${top100HoldPercent}%` : "--",
            amount: top100HoldAmount ? toFormat(new BigNumber(top100HoldAmount).toFixed(2)) : "--",
            symbol: token?.symbol,
          })}
          content1={`${t("common.token.total.supply.colon")} ${
            totalSupply ? toFormat(new BigNumber(totalSupply).toFixed(2)) : "--"
          } ${token?.symbol}`}
          content2={`${t("common.token.total.holders.colon")} ${totalHolders ?? "--"}`}
        />
