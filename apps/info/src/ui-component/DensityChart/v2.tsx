...
import { useTicksSurroundingPrice, TickProcessed } from "hooks/swap/v2/usePoolTickChartData";
...
export default function DensityChart({ address, token0Price }: DensityChartProps) {
  ...
  const { result: _pool } = useSwapPoolInfo(address);
  const [, token0] = useToken(_pool?.token0);
  const [, token1] = useToken(_pool?.token1);
