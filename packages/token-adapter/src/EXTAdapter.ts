...
export class EXTTokenAdapter extends BaseTokenAdapter<EXTToken> {
  ...
  public async totalHolders({ canisterId }: TotalHoldersRequest) {
    return resultFormat<bigint>(
      await (await this.actor(canisterId)).totalHolders()
    );
  }
  ...
  public async balance({ canisterId, params }: BalanceRequest) {
    ...
    if (params.user.principal) {
      const sub = params.subaccount
        ? SubAccount.fromBytes(Uint8Array.from(params.subaccount))
        : undefined;
      address = principalToAccount(
        params.user.principal.toString(),
        isOkSubAccount(sub) ? sub : undefined
      );
  ...
  public async setFee({ canisterId, identity, params }: SetFeeRequest) {
    return resultFormat<boolean>(
      await (await this.actor(canisterId, identity)).setFee(params)
    );
  }
  ...
  public async setFeeTo({ canisterId, identity, params }: SetFeeToRequest) {
    ...
    return resultFormat<boolean>(
      await (
        await this.actor(canisterId, identity)
      ).setFeeTo({ address: params.address })
    );
  }
  ...
  public async transactions({ canisterId, params }: TransactionRequest) {
    ...
    if (cap_id) {
      ...
      return resultFormat<PaginationResult<Transaction>>(
        await params.getCapTransactions(
          cap_id,
          params.witness ?? false,
          params.offset
        )
    ...
    return resultFormat<PaginationResult<Transaction>>(
      await (
        ...
        user: availableArgsNull<TokenUser>(
          params.user?.address ? { address: params.user.address } : undefined
        ),
        offset: availableArgsNull<bigint>(
          params.offset ? BigInt(params.offset) : null
        ),
        limit: availableArgsNull<bigint>(
          params.limit ? BigInt(params.limit) : null
        ),
        index: availableArgsNull<bigint>(
          params.index ? BigInt(params.index) : null
        ),
      })
    );
  }
    ...
    return resultFormat<bigint>(
      await (
        ...
        owner: params.owner.address
          ...
          : { principal: params.owner.principal! },
        subaccount: params.subaccount ? [Array.from(params.subaccount)] : [],
        spender: params.spender,
      })
    );
  ...
  public async metadata({ canisterId }: MetadataRequest) {
    ...
    const logo = resultFormat<string>(
      await (await this.actor(canisterId)).logo()
    ).data;
    const fee = resultFormat<bigint>(
      await (await this.actor(canisterId)).getFee()
    ).data;
  ...
  public async setLogo({ canisterId, params, identity }: SetLogoRequest) {
    return resultFormat<boolean>(
      await (await this.actor(canisterId, identity)).setLogo(params)
    );
  }
