...
export async function getInfoPool(storageId: string, poolId: string) {
  return resultFormat<PublicPoolOverView>(
    await (await poolStorage(storageId)).getPool(poolId)
  ).data;
}
...
export async function getInfoPoolChartData(
  storageId: string,
  poolId: string,
  offset: number,
  limit: number
  ...
  return resultFormat<PublicPoolChartDayData[]>(
    await (
      await poolStorage(storageId)
    ).getPoolChartData(poolId, BigInt(offset), BigInt(limit))
  ).data;
}
...
export async function getInfoPoolTransactions(
  storageId: string,
  poolId: string,
  offset: number,
  limit: number
  ...
  return resultFormat<PoolStorageTransaction[]>(
    await (
      await poolStorage(storageId)
    ).getPoolTransactions(poolId, BigInt(offset), BigInt(limit))
  ).data;
}
