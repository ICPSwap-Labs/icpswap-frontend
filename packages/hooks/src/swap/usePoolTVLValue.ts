...
export function usePoolTVLValue({ pool }: UsePoolTVLValueProps): string | undefined {
  ...
  return useMemo(() => {
    if (isNullArgs(infoAllTokens) || isNullArgs(token0Balance) || isNullArgs(token1Balance)) return undefined;
    ...
    return parseTokenAmount(token0Balance, token0.decimals)
      ...
      .toString();
  }, [infoAllTokens, token0Balance, token1Balance]);
}
