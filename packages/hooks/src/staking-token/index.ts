...
/* token controller */
export async function createStakingTokenPool(
  args: CreateTokenPoolArgs,
  identity: ActorIdentity
) {
  return resultFormat<string>(
    await (await stakingTokenController(identity)).createTokenPool(args)
  );
}

export async function getStakingTokenPools(
...
export function useStakingTokenPools(
  state: bigint | undefined,
  offset: number,
  limit: number
) {
...
export async function getStakingTokenGlobalData() {
  return resultFormat<StakingPoolGlobalData>(
    await (await stakingTokenController()).getTokenPoolsGlobalData()
  ).data;
}
...
export async function getStakingPoolFromController(canisterId: string) {
  return resultFormat<StakingPoolControllerPoolInfo>(
    await (
      await stakingTokenController()
    ).getPoolInfo(Principal.fromText(canisterId))
  ).data;
}
...
export async function getStakingTokenPool(canisterId: string) {
  return resultFormat<StakingTokenPoolInfo>(
    await (await stakingToken(canisterId)).getPoolInfo()
  ).data;
}
...
export async function getStakingTokenUserInfo(
  canisterId: string,
...
export async function getStakingTokenAllUserInfo(
  canisterId: string,
  offset: number,
  limit: number
) {
  return resultFormat<StakingPoolUserInfo>(
    await (
      await stakingToken(canisterId)
    ).findAllUserInfo(BigInt(offset), BigInt(limit))
  ).data;
}
...
export async function getStakingTokenCycles(canisterId: string) {
  return resultFormat<StakingPoolCycle>(
    await (await stakingToken(canisterId)).getCycleInfo()
  ).data;
}

export function useStakingTokenCycles(
  canisterId: string | undefined,
  reload?: boolean
) {
  return useCallsData(
...
export async function stakingTokenClaim(
  canisterId: string,
  identity: ActorIdentity
) {
  return resultFormat<string>(
    await (await stakingToken(canisterId, identity)).claim()
  );
}

export async function stakingTokenDeposit(
  canisterId: string,
  identity: ActorIdentity
) {
  return resultFormat<string>(
    await (await stakingToken(canisterId, identity)).deposit()
  );
}

export async function stakingTokenDepositFrom(
  canisterId: string,
  identity: ActorIdentity,
  amount: bigint
) {
  return resultFormat<string>(
    await (await stakingToken(canisterId, identity)).depositFrom(amount)
  );
}

export async function stakingTokenHarvest(
  canisterId: string,
  identity: ActorIdentity
) {
  return resultFormat<bigint>(
    await (await stakingToken(canisterId, identity)).harvest()
  );
}

export async function stakingTokenWithdraw(
  canisterId: string,
  identity: ActorIdentity,
  amount: bigint
) {
  return resultFormat<string>(
    await (await stakingToken(canisterId, identity)).withdraw(amount)
  );
}
...
export async function getStakingTokenTransactions(
  ...
  return resultFormat<PaginationResult<StakingPoolTransaction>>(
    await (
      await stakingTokenStorage(canisterId)
